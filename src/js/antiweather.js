		...
		search: (input) => {
			if (input.length >= 2) {
				return locations.filter((location) => {
					return location.h.toLowerCase().startsWith(input.toLowerCase());
				});
			} else {
				return [];
			}
